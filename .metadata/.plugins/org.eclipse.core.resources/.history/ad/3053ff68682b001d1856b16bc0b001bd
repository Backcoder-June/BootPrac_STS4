...
public class UploadController {
	...
	public String testing2(Model model) {
		...
		String getit = "getitfromcontroller";
		model.addAttribute("getit", getit);
		return "modeltest";
	}
	
	@GetMapping("/uploadname")
	public String uploadname() {
		return "upload/uploadname";
	}
	
	@GetMapping("/uploadname2")
	public String uploadname(String name, Model model) {
		System.out.println(name);
		List<UploadForm> uploadlist = uploadservice.fbn(name); 
		
		ArrayList<String> filelist = new ArrayList<>();
		for (UploadForm uploadForm : uploadlist) {
			if (uploadForm.getFile1().contains("png")) {
			filelist.add(uploadForm.getFile1());
		} 
		}
	...
	public String uploadform() {
	  ...
	  public String uploadprocess(UploadDTO dto, Model model) throws IOException {
		  ...
		  UploadForm form = new UploadForm(dto.getName(), dto.getDescription(), newname, dto.getUploadtime());
		  uploadservice.insertUpload(form);
