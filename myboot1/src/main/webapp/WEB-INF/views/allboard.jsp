...
<br>
<hr>
<form action="http://localhost:8081/searchboard">
<select name="item">
<option>번호</option>
<option>제목</option>
<option>내용</option>
...
<table border="5">
  ...
  <c:forEach items="${allboard}" var="board" >
    <tr>
      <th>${board.id}</th>
      <td><a href="http://localhost:8081/myboard?id=${board.id}">${board.title}</a></td>
      <td>${board.writer}</td>
...
<br> <a href='http://localhost:8081/logined'>홈으로</a>
